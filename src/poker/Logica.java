...
public class Logica extends Interfaz implements ActionListener {
	...
	public void JugarDeNuevo() {
		if (humano.getBalance() > 0 && pc.getBalance() > 0) {
			...
			JOptionPane.showMessageDialog(null,
					"Ya no tienes dinero para seguir jugando, lo siento, chico, creo que te toca pagar con la vida, nada personal...",
	...
	public void analizarCartaAlta(Jugador jugador) {
		...
		int jugada[] = { CARTAALTA, cartas.get(0).obtenerCosto() };
	...
	public void analizarColor(Jugador jugador) {
		int[] palos = new int[4];
		List<Carta> cartas = new ArrayList<Carta>(); // Llena de ceros el array
		for (int i = 0; i < 4; i++) {
			palos[i] = 0;
		}
		...
		for (int pos = 0; pos < 4; pos++) {
			if (palos[pos] >= 5) {
				int pesoMano = cartas.get(0).obtenerCosto();
				System.out.println("Color");
				int jugada[] = { COLOR, pesoMano };
				jugador.setJugada(jugada);
			}
		}
	}
	...
	public void analizarRepetidas(Jugador jugador) {
		...
		for (int pos = 13; pos > 0; pos--) {
			if (cartas[pos] == 2) {
				...
				trioDe = Carta.obtenerValorCarta(pos);
				valorTrio = Carta.costoCarta(pos);
				trio = true;
			} else if (cartas[pos] == 4) {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == retirarse) {// se retira de la mano, mas no del juego, se le da todo lo del bote al pc y se
											...
											// gana el computador y se reinicia el juego, el juego acaba cuando alguno
											// se
											// quede sin dinero
	...
	public void determinarGanador() {
		List<Carta> cartasHumano = new ArrayList<Carta>(); // crea una lista de las cartas porque hay un problema si le
		List<Carta> cartasPC = new ArrayList<Carta>();// pasamos
		// directamente la mano al sort
		for (int i = 0; i < 7; i++) {
			cartasHumano.add(humano.getMano().getCarta(i));
			cartasPC.add(pc.getMano().getCarta(i));
		}

		Collections.sort(cartasHumano, Collections.reverseOrder());
		Collections.sort(cartasPC, Collections.reverseOrder());

		outerloop: for (int i = 0; i < 2; i++) {
			if (humano.getJugada()[0] == PAR && pc.getJugada()[0] == PAR
					&& humano.getJugada()[1] == pc.getJugada()[1]) {
				for (int e = 0; e < 7; e++) {
					if (cartasHumano.get(e).obtenerCosto() > cartasPC.get(e).obtenerCosto()) {
						humano.adicionarDinero(Integer.parseInt(bote));
						JOptionPane.showMessageDialog(null, "Ganas con par", "Alerta", JOptionPane.WARNING_MESSAGE);
						break outerloop;
					} else if (cartasHumano.get(e).obtenerCosto() < cartasPC.get(e).obtenerCosto()) {
						pc.adicionarDinero(Integer.parseInt(bote));
						JOptionPane.showMessageDialog(null, "El PC gana con par ", "Alerta",
								JOptionPane.WARNING_MESSAGE);
						break outerloop;
					}
			...
			if (humano.getJugada()[0] == CARTAALTA && pc.getJugada()[0] == CARTAALTA
					&& humano.getJugada()[1] == pc.getJugada()[1]) {
				for (int e = 0; e < 7; e++) {
					if (cartasHumano.get(e).obtenerCosto() > cartasPC.get(e).obtenerCosto()) {
						humano.adicionarDinero(Integer.parseInt(bote));
						JOptionPane.showMessageDialog(null, "Ganas con carta alta", "Alerta",
								JOptionPane.WARNING_MESSAGE);
						break outerloop;
					} else if (cartasHumano.get(e).obtenerCosto() < cartasPC.get(e).obtenerCosto()) {
						pc.adicionarDinero(Integer.parseInt(bote));
						JOptionPane.showMessageDialog(null, "El PC gana con carta alta ", "Alerta",
								JOptionPane.WARNING_MESSAGE);
						break outerloop;
					}
